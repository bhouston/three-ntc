...
class NTCLoader extends Loader {
  ...
  parse(data: NTCManifest | string): NTCParsedAsset {
    ...
    const cpuModel: NTCCpuModel = {
      ...
      positionalEncodingPeriod: manifest.latents.positionalEncodingPeriod,
      textureResolution: manifest.latents.textureResolution ?? 2 ** manifest.latents.maxLod,
      lodOffset: manifest.latents.lodOffset ?? 0,
      grids,
      lowResGrids: manifest.latents.lowResLevels?.map((level, index) =>
        decodeLevel(level, `latents.lowResLevels[${index}]`),
      ),
      decoder: { layers: decoderLayers },
      outputChannels:
        manifest.outputChannels !== undefined
          ? manifest.outputChannels
          : decoderLayers[decoderLayers.length - 1].outputSize,
...
function decodeChannelClassification(
  channels: NTCManifest['channels'],
  renderFlags: NTCManifest['renderFlags'],
): NTCChannelClassification {
  ...
  return {
    ...
    totalChannels,
    packCount,
    constantValues: channels.constantValues || {},
    renderFlags: renderFlags || null,
  };
}
...
function validateManifest(manifest: NTCManifest): void {
  ...
  if (
    manifest.uvTransform !== undefined &&
