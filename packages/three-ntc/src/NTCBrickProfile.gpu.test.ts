...
it('profiles the shipped brick in a standalone 1024px viewer without training', async () => {
  ...
  scene.add(new Mesh(geometry, material), new AmbientLight(0xffffff, 0.6));
  ...
  try {
    ...
    for (let group = 0; group < Math.ceil(cpuModel.outputChannels / 4); group++) {
      const values = await renderNodeToFloats(
        renderer,
        vec4(...Array.from({ length: 4 }, (_, i) => raw[group * 4 + i] ?? float(0))),
        16,
      );
      for (let y = 0; y < 16; y++)
        for (let x = 0; x < 16; x++) {
          const [u, v] = pixelUv(16, x, y),
            expected = evaluateNTCCpu(cpuModel, u, v, 0);
    ...
    await commands.recordMetric({
      ...
      rawCpuMse: squaredError / observations,
      model: cpuModel.decoder.layers.map((l) => [l.inputSize, l.outputSize]),
      firstFrameMs: times[0],
      steadyFrameMs: times.slice(1),
      errors,
    });
  } finally {
    alive = false;
