...
function privateStorageUpperBound(code: string): number {
  ...
  const size = (type: string): number => {
    ...
    const matrix = type.match(/^mat([234])x[234]<f32>$/);
    if (matrix) return Number(matrix[1]) * 16;
    const array = type.match(/^array<(.+),\s*(\d+)u?>$/);
    if (array) return size(array[1]) * Number(array[2]);
...
for (const hiddenSize of [32, 64])
  it(`keeps the ${hiddenSize}-wide full physical material below the 8 KB private-storage limit`, async () => {
    const times: Record<string, number> = {};
    let start = performance.now();
    const renderer = await getRenderer();
    times.init = performance.now() - start;
    ...
    const model = makeModel(7, {
      ...
      outputChannels: layout.totalChannels,
    });
    ...
    scene.add(new Mesh(geometry, material), new AmbientLight(0xffffff, 1));
    ...
    try {
      ...
      times.compile = performance.now() - start;
      start = performance.now();
    ...
    await commands.recordMetric({
      ...
      kind: 'runtime-shader',
      hiddenSize,
      privateBytesUpperBound: bytes,
      mse: squaredError / expected.length,
    });
  });
