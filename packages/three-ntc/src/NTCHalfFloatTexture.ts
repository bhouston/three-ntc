 ...
 */
function createHalfFloatLatentTexture(
  ...
  for (let g = 0; g < grids.length; g++) {
    ...
    for (let p = 0; p < grid.width * grid.height; p++)
      for (let c = 0; c < grid.channels; c++) {
        packed[(Math.floor(c / 4) * grid.width * grid.height + p) * 4 + (c % 4)] = DataUtils.toHalfFloat(
          grid.data[p * grid.channels + c],
        );
      }
    texture.needsUpdate = true;
  }
...
function buildLevelTextures(cpuModel: NTCMipChainModel) {
  const textures = [...cpuModel.grids, ...(cpuModel.lowResGrids || [])].map((grid) => {
    ...
    const texture = new DataArrayTexture(
      new Uint16Array(grid.width * grid.height * depth * 4),
      grid.width,
      grid.height,
      depth,
    );
