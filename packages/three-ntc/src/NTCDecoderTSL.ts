...
function gridFeatures(uv: any, grid: NTCGrid, texture: any, concatenate: boolean): { values: any[]; tx: any; ty: any } {
  ...
  const x0 = floor(x),
    ...
    ty = y.sub(y0);
  const weights = [
    float(1).sub(tx).mul(float(1).sub(ty)),
    tx.mul(float(1).sub(ty)),
...
function sampleFeatures(uv: any, model: NTCCpuModel, textures: any[], lod: any): any[] {
  ...
  for (let g = 0; g < model.grids.length; g++) {
    ...
    if (model.dualGrid) {
      ...
      for (let c = 0; c < lowChannels; c++)
        features[highWidth + c] = features[highWidth + c].add(low.values[c].mul(gate));
    }
  }
  return features;
}
...
function evaluateNeuralTextureSampled(
  ...
  const sampled = Fn(() => {
    ...
    if (samplingMode !== 'trilinear') {
      ...
      const size = floor(float(model.textureResolution ?? 2 ** model.maxLod).div(pow(2, level)))
        .max(1)
        .toVar();
    ...
    const lower = floor(resolved).toVar(),
      upper = lower.add(1).min(model.maxLod).toVar();
    ...
    Loop(
      ...
      ({ ntcTrilinearTap: i }: { ntcTrilinearTap: any }) => {
        ...
        const center = base
          .add(vec2(float(x).add(0.5), float(y).add(0.5)))
          .div(size)
          .fract();
        const raw = evaluateNeuralTextureRaw(center, model, null, null, level, textures, packed);
        const weight = x
          .equal(1)
          .select(fraction.x, fraction.x.oneMinus())
          .mul(y.equal(1).select(fraction.y, fraction.y.oneMinus()))
          .mul(firstMip.select(blend.oneMinus(), blend));
        for (let c = 0; c < model.outputChannels; c++)
          result.element(c).addAssign(applyChannelActivation(raw[c], activations[c]).mul(weight));
      },
    );
    return result;
  })();
...
export {
  ...
  evaluateNeuralTextureFiltered,
  evaluateNeuralTextureRaw,
  buildMipChainTexture,
  buildLevelTextures,
};
