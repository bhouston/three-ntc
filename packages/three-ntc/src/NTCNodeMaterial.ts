...
import {
  buildLevelTextures,
  packDecoder,
  evaluateNeuralTextureSampled,
  NTC_SAMPLING_MODES,
  type NTCSamplingMode,
  NTCCpuModel,
} from './NTCDecoderTSL.js';
...
function sliceChannels(outputs: any[], activeChannels: NTCLayoutChannel[], activated = false): Record<string, any> {
  ...
  for (const channel of activeChannels) {
    ...
    for (let i = 0; i < channel.size; i++)
      values.push(
        activated
          ? outputs[channel.offset + i]
          : applyChannelActivation(outputs[channel.offset + i], channel.activation),
      );
...
class NTCNodeMaterial extends (MeshPhysicalNodeMaterial as any) {
  ...
  constructor(
    ...
    options: NTCNodeMaterialOptions = {},
  ) {
    super();
    ...
    if (!NTC_SAMPLING_MODES.includes(this._samplingMode))
      throw new Error(`Unknown NTC sampling mode: ${this._samplingMode}`);
    ...
    this._lodNode =
      options.lodNode || computeAutoLodNode(coord, cpuModel.maxLod, this._lodBiasUniform, cpuModel.textureResolution);
  ...
  private _buildSamplingGraph(initialize = false): void {
    const cpuModel = this.cpuModel,
      activeChannels = this.activeChannels;
    const channels = this.channels,
      constantValues = this._constantValues;
    ...
    if (this._samplingMode === 'stochastic') {
      ...
      const seed = uint(screenCoordinate.x)
        .mul(1973)
        .add(uint(screenCoordinate.y).mul(9277))
        .add(uint(frameId).mul(26699));
    ...
    const outputs = evaluateNeuralTextureSampled(
      this._localUv,
      cpuModel,
      this.levelTextures!,
      this._lodNode,
  ...
  get samplingMode(): NTCSamplingMode {
  ...
  setSamplingMode(mode: NTCSamplingMode): void {
...
function modelShape(model: NTCCpuModel): string {
  return JSON.stringify({
    ...
    levels: model.levels,
    mipsPerLevel: model.mipsPerLevel,
    maxLod: model.maxLod,
    lodOffset: model.lodOffset,
    textureResolution: model.textureResolution,
    positionalEncoding: model.positionalEncoding,
    positionalEncodingPeriod: model.positionalEncodingPeriod,
    dualGrid: model.dualGrid,
