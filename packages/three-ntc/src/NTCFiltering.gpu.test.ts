...
for (const mipBlend of [false, true]) {
  it(`filters activated decoded values across ${mipBlend ? 'mips' : 'texels'}`, async () => {
    ...
    layer.biases.fill(mipBlend ? -2 : 0);
    for (let c = 0; c < 4; c++)
      layer.weights[c * layer.inputSize + (mipBlend ? layer.inputSize - 1 : 0)] = mipBlend ? 6 : 1;
    const textures = buildLevelTextures(model);
    const sigmoid = (x: number) => 1 / (1 + Math.exp(-x));
    const values = evaluateNeuralTextureFiltered(vec2(0.5, 0.25), model, textures, float(mipBlend ? 0.5 : 0), [
      'sigmoid',
      'sigmoid',
      'sigmoid',
      'sigmoid',
    ...
    const nearest = evaluateNeuralTextureSampled(
      vec2(0.5, 0.25),
      model,
      textures,
      float(mipBlend ? 0.5 : 0),
