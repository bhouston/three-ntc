  ...
  renderer.compute(
    Fn(() => {
      ...
      for (let stream = 0; stream < 5; stream++)
        buffer.element(s.mul(5).add(stream)).assign(trainingRandomTSL(s, float(10000), stream));
    })().compute(1024),
  );
  const data = new Float32Array(await renderer.getArrayBufferAsync(attr));
  for (let s = 0; s < 1024; s++)
    for (let stream = 0; stream < 5; stream++) expect(data[s * 5 + stream]).toBe(trainingRandom(s, 10000, stream));
  attr.dispose();
});
