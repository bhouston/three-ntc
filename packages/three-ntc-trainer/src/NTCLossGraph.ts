...
class NTCLossGraph {
  ...
  constructor(
    canvas: HTMLCanvasElement,
    series: NTCLossSeries[],
    { legend = null, ips = null }: { legend?: HTMLElement | null; ips?: HTMLElement | null } = {},
  ) {
    this.canvas = canvas;
  ...
  draw() {
    ...
    context.fillText(
      Number.isFinite(primaryValue) ? (primaryValue as number).toExponential(3) : '—',
      width - marginRight,
      9,
    );
    ...
    context.fillStyle = '#aaa';
    context.fillText(
      lastIteration !== null ? `iteration ${lastIteration}` : 'iteration',
      width - marginRight,
      height - 4,
    );
