...
const createMXElement = ( name: string, nodeFunc: ( ...args: any[] ) => any, params: string[] = [], defaults: Record<string, any> = {}, usesNode = false ): MXElement => ( { name, nodeFunc, params, defaults, usesNode } );
...
const mx_difference = ( fg: any, bg: any, mixval: any = 1 ) => add( mul( mixval, abs( sub( bg, fg ) ) ), mul( sub( 1, mixval ), bg ) );
...
const applyBlendByChannel = ( channelFunc: ( fg: any, bg: any, mixval: any ) => any, fg: any, bg: any, mixval: any = 1 ) => {
...
const mx_ramp = ( texcoord: any = vec2( 0, 0 ), type: any = 0, interpolation: any = 1, numIntervals: any = 2, ...rest: any[] ) => {
...
const defaultFloat = ( value: number ) => () => float( value );
const defaultInt = ( value: number ) => () => int( value );
const defaultBool = ( value: boolean ) => () => bool( value );
const defaultColor = ( r: number, g: number, b: number ) => () => color( r, g, b );
...
const mx_fractal_noise_materialx_2d = ( texcoord: any, octaves: any, lacunarity: any, diminish: any, amplitude: any, nodeX: any ) =>
...
const mx_fractal_noise_materialx_3d = ( position: any, octaves: any, lacunarity: any, diminish: any, amplitude: any, nodeX: any ) =>
...
const mx_fractal_noise_float_materialx_2d = Fn( ( [ texcoordInput, octavesInput, lacunarityInput, diminishInput, amplitudeInput ]: any[] ) => {
...
const mx_fractal_noise_vec3_materialx_2d = Fn( ( [ texcoordInput, octavesInput, lacunarityInput, diminishInput, amplitudeInput ]: any[] ) => {
...
const MtlXLibrary: Record<string, MXElement> = Object.fromEntries( MXElements.map( ( entry ) => [ entry.name, entry ] ) );
