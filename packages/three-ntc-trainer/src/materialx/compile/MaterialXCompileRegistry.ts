...
const register = ( registry: Map<string, any>, categories: string[], handler: ( nodeX: any, out: any, compileContext: MaterialXCompileContext ) => any ) => {
...
const applyTextureAddressModeDefault = ( node: any, uvNode: any, addressModes: { u: string; v: string }, defaultNode: any ) => {
...
const sampleTexture = ( textureFile: any, uvNode: any, compileContext: MaterialXCompileContext, fallback: any, addressModes: { u: string; v: string } | null = null, defaultNode: any = fallback ) => {
...
const getSwitchBranchNode = ( nodeX: any, index: number ) => nodeX.getNodeByName( `in${index}` ) || getZeroNodeForType( nodeX.type );
...
const compileNormalizedSpaceInputNode = ( nodeX: any, objectNode: any, worldNode: any ) => normalize( compileSpaceInputNode( nodeX, objectNode, worldNode ) );
...
function createMaterialXCompileRegistry() {
	...
	register( registry, [ 'gltf_colorimage' ], ( nodeX, _out, compileContext ) => compileGltfColorImageNode( nodeX, _out, compileContext ) );
	...
	register( registry, [ 'transformmatrix' ], ( nodeX, _out, compileContext ) => compileTransformMatrixNode( nodeX, compileContext ) );
