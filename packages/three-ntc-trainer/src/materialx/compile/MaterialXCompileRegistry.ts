...
const UV_FALLBACK_CATEGORIES = new Set([
  'checkerboard',
  'noise2d',
  'fractal2d',
  'cellnoise2d',
...
const getZeroNodeForType = (type: string) => {
...
const compileArtisticIorNode = (nodeX: any, out: any) => {
  ...
  const k2 = max(
    div(sub(mul(mul(iorPlusOne, iorPlusOne), reflectivity), mul(iorMinusOne, iorMinusOne)), sub(one, reflectivity)),
    vec3(0, 0, 0),
  );
...
const compileRemapNode = (nodeX: any) => {
  ...
  const remapped = add(mul(div(sub(inNode, inLow), safeDenominator), sub(outHigh, outLow)), outLow);
...
const compileSwitchNode = (nodeX: any) => {
  ...
  const whichNode = clamp(float(switchIndex), float(SWITCH_MIN_INDEX), float(SWITCH_MAX_INDEX));
...
const compileHexTiledNormalMapNode = (nodeX: any, compileContext: MaterialXCompileContext, sampleNode: any) => {
  ...
  const args = normalMapNodeElement.params.map((paramName: string) => {
...
const compileHexTiledTextureNode = (nodeX: any, compileContext: MaterialXCompileContext, category: string) => {
  ...
  const tileData = compileContext.mxHextileCoord(
    transformedUv,
    rotation,
    rotationRange,
    scale,
  ...
  let sample0 = texture(textureFile, tileData.coords[0]).grad(tileData.ddx[0], tileData.ddy[0]);
  let sample1 = texture(textureFile, tileData.coords[1]).grad(tileData.ddx[1], tileData.ddy[1]);
  let sample2 = texture(textureFile, tileData.coords[2]).grad(tileData.ddx[2], tileData.ddy[2]);
...
const compileGltfTextureNode = (nodeX: any, compileContext: MaterialXCompileContext, category: string) => {
  ...
  const node = applyTextureColorSpace(
    sampleTexture(textureFile, transformedUv, compileContext, fallback, addressModes, fallback),
    file,
  );
...
const compileInvertMatrix3Node = (matrixNode: any) => {
  ...
  return mat3(
    div(inv00, determinant),
    div(inv10, determinant),
    div(inv20, determinant),
    div(inv01, determinant),
    div(inv11, determinant),
    div(inv21, determinant),
    div(inv02, determinant),
    div(inv12, determinant),
    div(inv22, determinant),
  );
};
...
const compileInvertMatrix4Node = (matrixNode: any) => {
  ...
  const getMinor3 = (skipRow: number, skipColumn: number) => {
...
const compileInvertMatrixNode = (nodeX: any, compileContext: MaterialXCompileContext) => {
  ...
  if (isMatrixType) {
    const size = matrixType === 'matrix33' ? 3 : 4;
    const fallback =
      size === 3 ? mat3(...compileContext.IDENTITY_MAT3_VALUES) : mat4(...compileContext.IDENTITY_MAT4_VALUES);
...
function createMaterialXCompileRegistry() {
  ...
  register(registry, ['texcoord'], (nodeX, _out, compileContext) => compileTexcoordNode(nodeX, compileContext));
  register(registry, ['geomcolor'], (nodeX) => compileGeomColorNode(nodeX));
  register(registry, ['tiledimage'], (nodeX, _out, compileContext) => compileTiledImageNode(nodeX, compileContext));
  register(registry, ['image'], (nodeX, _out, compileContext) => compileImageLikeNode(nodeX, compileContext));
  register(registry, ['hextiledimage', 'hextilednormalmap'], (nodeX, _out, compileContext) =>
    compileHexTiledTextureNode(nodeX, compileContext, nodeX.element),
  );
  register(registry, ['gltf_image', 'gltf_normalmap'], (nodeX, _out, compileContext) =>
    compileGltfTextureNode(nodeX, compileContext, nodeX.element),
  );
  register(registry, ['gltf_colorimage'], (nodeX, _out, compileContext) =>
    compileGltfColorImageNode(nodeX, _out, compileContext),
  );
  register(registry, ['gltf_anisotropy_image'], (nodeX, _out, compileContext) =>
    compileGltfAnisotropyImageNode(nodeX, _out, compileContext),
  );
  register(registry, ['gltf_iridescence_thickness'], (nodeX, _out, compileContext) =>
    compileGltfIridescenceThicknessNode(nodeX, compileContext),
  );
  register(registry, ['switch'], (nodeX) => compileSwitchNode(nodeX));
  register(registry, ['transformmatrix'], (nodeX, _out, compileContext) =>
    compileTransformMatrixNode(nodeX, compileContext),
  );
  register(registry, ['creatematrix'], (nodeX) => compileCreateMatrixNode(nodeX));
  register(registry, ['invertmatrix'], (nodeX, _out, compileContext) => compileInvertMatrixNode(nodeX, compileContext));
  return registry;
}
