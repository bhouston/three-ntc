...
function typesCompatible(expectedType: string | null | undefined, actualType: string | null | undefined): boolean {
  ...
  if (expectedType === actualType) return true;
  if (
    (expectedType === 'float' && actualType === 'integer') ||
    (expectedType === 'integer' && actualType === 'float')
  ) {
    return true;
  }
...
function validatePortConnection(inputNodeX: MaterialXNode, parentNodeX: MaterialXNode, log: MaterialXLog): void {
  ...
  if (inputType && outputType && typesCompatible(inputType, outputType) === false) {
    log.add(MaterialXLogCodes.TYPE_MISMATCH, `Mismatched types in port connection: ${inputSnippet}`, parentNodeX.name);
  }
}
