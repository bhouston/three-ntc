...
function applyOpenPbrSurface(
  ...
  if (
    transmissionEnabled &&
    hasNodeValue(transmissionDepthNode) &&
...
const surfaceMapperRegistry = new Map(
  Object.entries(MaterialXSurfaceMappings).map(([category, apply]) => [category, { category, apply }]),
);
