...
class MaterialXLoader extends Loader {
  ...
  parse(text: string, options: MaterialXLoaderParseOptions = {}): MaterialXParseResult {
    ...
    const document = new MaterialXDocument(
      this.manager,
      options.path || this.path,
      log,
      options.archiveResolver || null,
      options.uvSpace,
    );
    ...
    if (options.throwOnErrors !== false && log.errors.length > 0) {
      ...
      throw new Error(
        `THREE.MaterialXLoader: MaterialX translation failed with ${log.errors.length} error(s). ${details}`,
      );
    }
