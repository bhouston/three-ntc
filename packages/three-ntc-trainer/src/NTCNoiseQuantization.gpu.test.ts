...
it('noise is applied to stored taps, matches the CPU counter stream, and switches off for frozen adaptation', async () => {
  ...
  for (let i = 0; i < 2048; i++) {
    const x = Math.floor(trainingRandom(i, 1, 0) * 32),
      y = Math.floor(trainingRandom(i, 1, 1) * 32);
...
it('a fixed degenerate quantization range stays finite on the GPU', async () => {
  ...
  const options = {
    ...
    batchSize: 1,
    outputChannels: 4,
    quantization: { mode: 'uint4', range: [0.25, 0.25] as [number, number] },
  };
  const gpu = new NTCGPUModel(options);
