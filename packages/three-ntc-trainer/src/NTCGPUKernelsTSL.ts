 ...
 * storage buffers.
 */
function hardGeluTSL(x: TSLNode): TSLNode {
 ...
 */
function hardGeluDerivativeTSL(x: TSLNode): TSLNode {
 ...
 */
function applyHiddenActivationTSL(z: TSLNode, activation: string): TSLNode {
 ...
 */
function activationDerivativeTSL(z: TSLNode, activation: string): TSLNode {
  if (activation === 'hgelu') return hardGeluDerivativeTSL(z);

  return select(z.greaterThan(0.0), float(1.0), float(0.0));
}

interface AdamParameterBuffers {
  attribute: InstanceType<typeof StorageBufferAttribute>;
  gradAttribute: InstanceType<typeof StorageBufferAttribute>;
  mAttribute: InstanceType<typeof StorageBufferAttribute>;
  vAttribute: InstanceType<typeof StorageBufferAttribute>;
  valuesStorage: TSLNode;
  gradAtomic: TSLNode;
  mStorage: TSLNode;
  vStorage: TSLNode;
}
 ...
 * per trainer.
 */
function createAdamParameterBuffers(count: number): AdamParameterBuffers {
 ...
 */
function accumulateDenseLayerGradTSL({
  activationsStorage,
  gradWeightsAtomic,
  deltaBase,
  inputBase,
  ...
  weightsOffset,
  biasesOffset,
  floatGradients = false,
}: {
  activationsStorage: TSLNode;
  gradWeightsAtomic: TSLNode;
  deltaBase: TSLNode;
  inputBase: TSLNode;
  inputSize: number;
  outputSize: number;
 ...
 */
function backwardDenseLayerTSL({
  activationsStorage,
  weightsStorage,
  deltaBase,
  deltaSize,
  ...
  prevSize: number;
  prevZBase: TSLNode;
  outDeltaBase: TSLNode;
  activation?: string;
}): void {
  Loop({ start: 0, end: prevSize, type: 'int', name: 'i', condition: '<' }, ({ i }: { i: TSLNode }) => {
    const gradInput_i = float(0.0).toVar();

    Loop({ start: 0, end: deltaSize, type: 'int', name: 'j', condition: '<' }, ({ j }: { j: TSLNode }) => {
      const delta_j = activationsStorage.element(deltaBase.add(j));
...
function computeGradientClipScale(
  gradNormAtomic: TSLNode,
  maxGradientNormUniform: TSLNode,
  floatGradients = false,
): TSLNode {
  const normSquared = floatGradients
    ? atomicLoadFloat(gradNormAtomic.element(0))
    : float(atomicLoad(gradNormAtomic.element(0))).div(float(GRADIENT_NORM_SCALE));
 ...
 */
function createResetGradientNormComputeNode(gpuModel: { gradNormAtomic: TSLNode }): TSLNode {
...
function createResetGradientsComputeNode(gpuModel: {
  layout: { totalWeights: number; totalLatents: number };
...
function createAdamComputeNode({
  ...
  name: string;
  floatGradients?: boolean;
  transform?: (value: TSLNode) => TSLNode;
}): TSLNode {
  return Fn(() => {
    const idx = int(instanceIndex).add(int(offset));
    let rawGrad = floatGradients
      ? atomicLoadFloat(gradAtomic.element(idx))
      : float(atomicLoad(gradAtomic.element(idx))).div(float(FIXED_POINT_SCALE));
    ...
    const nextM = float(beta1)
      .mul(m)
      .add(float(1.0 - beta1).mul(grad));
    const nextV = float(beta2)
      .mul(v)
      .add(
        float(1.0 - beta2)
          .mul(grad)
...
export type { AdamParameterBuffers, TSLNode };
