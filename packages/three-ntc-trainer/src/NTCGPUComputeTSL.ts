 ...
 * linear output.
 */
function createTextureTrainBatchComputeNode(
  gpuModel: NTCGPUModel,
  sourceTextures: TSLNode[],
  samples: { uv?: TSLNode; lod?: TSLNode; trainLatents?: boolean } = {},
): TSLNode {
  ...
  const quantizeLatent =
    quantization.mode !== 'none' ? QUANTIZATION_SCHEMES[quantization.mode].quantizeForwardTSL : null;
  ...
  return Fn(() => {
    ...
    for (let g = 0; g < gridLevels.length; g++) {
      ...
      const readTap = (offset: TSLNode, c: number): TSLNode => {
        ...
        if (quantization.method === 'noise') {
          ...
          return value.add(
            trainingRandomTSL(offset.add(c), stepUniform, 11)
              .sub(0.5)
              .mul(hi.sub(lo).div(bins - 1))
              .mul(gpuModel.quantizationNoiseUniform),
          );
        }
      ...
      const readBilinear = (c: number): TSLNode =>
        readTap(off0, c)
          .mul(w0)
          .add(readTap(off1, c).mul(w1))
          .add(readTap(off2, c).mul(w2))
          .add(readTap(off3, c).mul(w3));
    ...
    for (let l = 0; l < mlpLayers.length; l++) {
      ...
      forwardDenseLayerTSL({
        ...
        weightsOffset: layer.weightsOffset,
        biasesOffset: layer.biasesOffset,
        zBase,
        aBase,
    ...
    for (let l = mlpLayers.length - 1; l >= 0; l--) {
      ...
      if (l > 0) {
        ...
        backwardDenseLayerTSL({
          ...
          weightsStorage,
          deltaBase,
          deltaSize: layer.outputSize,
          weightsOffset: layer.weightsOffset,
          prevSize: layer.inputSize,
          prevZBase,
          outDeltaBase: prevDeltaBase,
          activation: mlpLayers[l - 1].activation,
        });
      } else {
    ...
    if (samples.trainLatents !== false) {
      ...
      for (let g = 0; g < gridLevels.length; g++) {
        ...
        if (gridLevels[g].isLowRes) {
          for (let c = 0; c < lowResChannels; c++)
            scatterBilinear(taps, c, activationsStorage.element(gradA0Base.add(g0Width + c)).mul(taps.weight));
          continue;
        }
        ...
        if (positionalEncoding) {
          ...
          for (let c = 0; c < channels; c++)
            scatterBilinear(taps, c, activationsStorage.element(gradA0Base.add(c)).mul(taps.weight));
        }
      }
    }
  })()
    .compute(batchSize)
    .setName('NTCTrainBatch');
}
...
function createAccumulateGradientNormComputeNode(gpuModel: NTCGPUModel, includeLatents = true): TSLNode {
  ...
  return Fn(() => {
    ...
    If(idx.lessThan(int(totalWeights)), () => {
      rawGrad.assign(
        gpuModel.floatGradients
          ? atomicLoadFloat(gradWeightsAtomic.element(idx))
          : float(atomicLoad(gradWeightsAtomic.element(idx))).div(float(FIXED_POINT_SCALE)),
      );
    }).Else(() => {
      const latentIdx = idx.sub(int(totalWeights));
      rawGrad.assign(
        gpuModel.floatGradients
          ? atomicLoadFloat(gradLatentsAtomic.element(latentIdx))
          : float(atomicLoad(gradLatentsAtomic.element(latentIdx))).div(float(FIXED_POINT_SCALE)),
      );
    });
    ...
    .setName('NTCAccumulateGradientNorm');
}
 ...
 */
function createTextureAdamWeightsComputeNode(
  gpuModel: NTCGPUModel,
  { beta1 = 0.9, beta2 = 0.999, epsilon = 1e-7 } = {},
): TSLNode {
  const { layout, learningRateUniform, stepUniform, gradNormAtomic, maxGradientNormUniform, invBatchUniform } =
    gpuModel;
 ...
 */
function createTextureAdamLatentsComputeNode(
  gpuModel: NTCGPUModel,
  { beta1 = 0.9, beta2 = 0.999, epsilon = 1e-7 } = {},
): TSLNode {
  const { layout, learningRateUniform, stepUniform, gradNormAtomic, maxGradientNormUniform, invBatchUniform } =
