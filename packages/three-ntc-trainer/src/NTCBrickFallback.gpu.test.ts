  ...
  try {
    for (const gradientPrecision of ['fixed', 'auto'] as const) {
      const trainer = new NTCTrainer({
        ...
        dualGrid: true,
        outputChannels: classification.totalChannels,
      ...
      const result = await trainer.train({
        renderer,
        sourceTextures: targets.map((t) => t.texture),
        onProgress: (p) => {
    ...
    await commands.recordMetric({
      kind: 'default-brick-safari-fallback',
      sourceResolution: 1024,
      batchSize: 8192,
      scheduleIterations: 10000,
      simulatedCompilerFailure: true,
      metrics,
    });
  } finally {
