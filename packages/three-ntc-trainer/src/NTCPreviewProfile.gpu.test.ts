...
it('profiles five default-size training steps with two live physical preview updates', async () => {
  ...
  const scene = new Scene(),
    camera = new OrthographicCamera(-1, 1, 1, -1, 0, 4);
  camera.position.z = 2;
  ...
  const heartbeat = setInterval(() => {
    ...
    maxHeartbeatGap = Math.max(maxHeartbeatGap, now - lastHeartbeat);
    lastHeartbeat = now;
    heartbeats++;
  }, 16);
  const device = viewing.backend.device,
  ...
  try {
    await trainer.train({
      ...
      onProgress: (p) => {
        ...
        const metric = {
          ...
          loss: p.loss,
          submissionMs: performance.now() - frameStart,
          completedMs: 0,
        };
        progress.push(metric);
        completions.push(
          device.queue.onSubmittedWorkDone().then(() => {
            metric.completedMs = performance.now() - frameStart;
          }),
        );
      },
    });
    ...
    await commands.recordMetric({
      ...
      animationIntervals,
      animationSummary: animationIntervals.length ? summarizeTimings(animationIntervals) : null,
      adapter: Object.fromEntries(
        ['vendor', 'architecture', 'device', 'description'].map((key) => [key, device.adapterInfo?.[key]]),
      ),
      elapsedMs: performance.now() - start,
      maxHeartbeatGap,
      heartbeats,
      progress,
      steadyFrameMs: frames,
      shaderModules,
      errors,
    });
  } finally {
    ...
    [training, viewing].forEach((renderer, i) =>
      renderer.backend.device.removeEventListener('uncapturederror', listeners[i]),
    );
    material?.dispose();
    geometry.dispose();
    target.dispose();
