...
it('fits and reloads HDR emission beyond the legacy sigmoid ceiling', async () => {
  ...
  const source = await bakeColorNodeToTexture(
    renderer,
    vec4(uv().x.mul(4).add(2), uv().y.mul(6).add(2), float(4), float(0)),
    size,
    { generateMipmaps: true },
  );
  const target = await renderNodeToFloats(
    renderer,
    vec4(uv().x.mul(4).add(2), uv().y.mul(6).add(2), float(4), float(0)),
    size,
  );
  ...
  try {
    for (const mode of ['legacy', 'hdr'] as const) {
      ...
      const options = {
        ...
        outputChannels: 3,
        channelActivations: buildChannelActivations(layout.channels),
        batchSize: 512,
        iterations: 420,
        seed: 7,
      ...
      try {
        ...
        const pixels = await renderNodeToFloats(
          renderer,
          vec4(...raw.map((v) => applyChannelActivation(v, activation)), float(0)),
          size,
        );
        let error = 0;
        for (let i = 0; i < pixels.length; i++) if (i % 4 !== 3) error += (pixels[i] - target[i]) ** 2;
