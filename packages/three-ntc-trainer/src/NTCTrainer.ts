...
class NTCTrainer {
  ...
  async train({ renderer, sourceTexture, sourceTextures, onProgress = null }: NTCTrainArgs) {
    ...
    const modelSettings = settings.textureResolution
      ? settings
      : {
          ...settings,
    ...
    try {
      ...
      const retrainIterations =
        quantization.mode === 'none' ? 0 : Math.round(iterations * (settings.retrainAfterQuantize as number));
      ...
      for (let iteration = 0; iteration < iterations; iteration++) {
        ...
        if (shouldSync) {
          [lastLoss] = await Promise.all([gpuModel.readLoss(renderer), gpuModel.syncToCPU(cpuModel, renderer)]);
        ...
        if (
          ...
          !latentsFrozen &&
          (iteration % QUANTIZATION_RANGE_REFRESH_INTERVAL === QUANTIZATION_RANGE_REFRESH_INTERVAL - 1 ||
            iteration === trainIterations - 1)
        ) {
