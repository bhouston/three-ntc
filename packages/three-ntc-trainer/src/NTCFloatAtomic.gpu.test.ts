...
for (const [count, contribution] of [
  [2048, 1e-7],
  [524288, -0.0625],
]) {
  it(`accumulates ${count} contributions of ${contribution} without integer overflow or truncation`, async () => {
    const r = await getRenderer();
    const buffers = createAdamParameterBuffers(1);
    try {
      r.compute(
        Fn(() => {
          atomicAddFloat(buffers.gradAtomic.element(0), float(contribution));
        })().compute(count),
      );
      ...
      disposeAdamParameterBuffers(buffers, r);
    }
  });
}
