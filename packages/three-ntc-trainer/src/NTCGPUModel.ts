...
class NTCGPUModel {
  ...
  constructor(options: NTCGPUModelOptions = {}) {
    ...
    if (options.gradientPrecision !== undefined && !['float', 'fixed'].includes(options.gradientPrecision))
      throw new Error('Invalid gradientPrecision');
    ...
    this.activationsAttribute = new StorageBufferAttribute(
      new Float32Array(batchSize * activationStride),
      1,
      Float32Array,
    );
    ...
    this.weightsLearningRateScale =
      options.weightsLearningRate === undefined ? 1 : options.weightsLearningRate / (options.learningRate ?? 0.01);
    if (!Number.isFinite(this.weightsLearningRateScale) || this.weightsLearningRateScale < 0)
      throw new Error('Invalid weightsLearningRate / learningRate ratio');
  ...
  async readLoss(renderer: any): Promise<number> {
    ...
    const loss = this.floatGradients
      ? new Float32Array(buffer)[0] / this.batchSize
      : array[0] / (FIXED_POINT_SCALE * this.batchSize);
