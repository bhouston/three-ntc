 ...
 */
function computeDecoderInputSize(
  channels: number,
  positionalEncoding: boolean,
  dualGrid: boolean = false,
  lowResChannels = channels,
): number {
  return (
    (positionalEncoding ? channels * 4 + POSITIONAL_ENCODING_SIZE : channels) + (dualGrid ? lowResChannels : 0) + 1
  );
}
...
function resolveNTCGridPyramidOptions(options: NTCGridPyramidOptions = {}): ResolvedNTCGridPyramidOptions {
  ...
  return {
    ...
    baseResolution:
      options.baseResolution || (textureResolution ? Math.min(textureResolution, MAX_GRID_RESOLUTION) : 128),
...
function createNTCGridPyramidModel(options: NTCGridPyramidOptions, random: () => number): NTCGridPyramidModel {
  ...
  const lowResGrids = dualGrid
    ? resolutions.map((r) =>
        createLatentGrid(Math.max(1, Math.floor(r / 2)), Math.max(1, Math.floor(r / 2)), lowResChannels, random),
      )
    : [];
