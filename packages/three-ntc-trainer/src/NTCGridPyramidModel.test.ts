...
describe('NTCGridPyramidModel', () => {
  it('allocates independent half-resolution G1 grids and round-trips wide features', () => {
    ...
    expect(loaded.lowResGrids?.map((g) => [g.width, g.channels])).toEqual([
      [8, 12],
      [2, 12],
      [1, 12],
    ]);
  ...
  it('adds one bilinear G1 tap when dualGrid is on, in either mode', () => {
    ...
    const dual = createNTCGridPyramidModel(
      { channels: 4, outputChannels: 3, levels: 2, baseResolution: 8, dualGrid: true },
      () => 0.5,
    );
    expect(dual.dualGrid).toBe(true);
    expect(dual.decoder.layers[0].inputSize).toBe(9);
    expect(
      computeTextureModelLayout({ channels: 4, outputChannels: 3, levels: 2, baseResolution: 8, dualGrid: true })
        .inputSize,
    ).toBe(9);
  });

  it('sizes the trained decoder MLP to match, and carries the flag on the model', () => {
    const plain = createNTCGridPyramidModel(
      { channels: 4, outputChannels: 3, levels: 2, baseResolution: 8 },
      () => 0.5,
    );
