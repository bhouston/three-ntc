...
it('GPU Lanczos mip uploads match the CPU filter and train without changing source ownership', async () => {
  ...
  const source = await bakeColorNodeToTexture(renderer, vec4(sin(uv().x.mul(12.56)), uv().y, -0.5, 0.3), 16);
  ...
  for (let lod = 0; lod < expected.length; lod++) {
    const actual = await renderNodeToFloats(renderer, textureLevel(texture, uv(), lod), expected[lod].width);
