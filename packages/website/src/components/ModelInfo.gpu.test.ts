...
it('starts Channels collapsed, counts only active channels, and expands the shared channel table', async () => {
  const info: ModelInfoData = {
    name: 'Example',
    size: { storageBytes: 100, memoryBytes: 200, mlpParams: 20, inputSize: 5, flopsPerDecode: 40 },
    grids: [{ group: 'G0', width: 4, height: 4, channels: 4, bits: 4 }],
    layers: [{ inputSize: 5, outputSize: 5 }],
    activeChannels: ['albedo', 'normal'],
    constantChannels: ['metalness'],
  };
  ...
  try {
    ...
    expect(
      [...fixture.querySelectorAll('tbody tr')].map((row) =>
