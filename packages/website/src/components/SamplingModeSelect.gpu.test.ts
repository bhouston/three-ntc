...
it('offers all runtime modes, starts at nearest, and applies changes to the material', async () => {
  ...
  try {
    ...
    expect([...select.options].map((option) => option.value)).toEqual(['nearest', 'stochastic', 'trilinear']);
    for (const mode of ['stochastic', 'trilinear', 'nearest'] as const) {
      ...
      if (mode === 'stochastic')
        await expect.poll(() => fixture.textContent).toContain('Noisy until temporal reconstruction');
    }
  } finally {
    root.unmount();
    fixture.remove();
