...
it('updates the visible card as the trainer form changes and restores values when toggled back', async () => {
  function Harness() {
    ...
    return createElement(
      'div',
      null,
      ...(['positionalEncoding', 'dualGrid'] as const).map((key) =>
        createElement(
          'button',
          {
            key,
            onClick: () => form.setFieldValue(key, !form.getFieldValue(key)),
          },
          key,
        ),
      ),
      ...(['uint8', 'uint4', 'uint2', 'none'] as const).map((mode) =>
        createElement(
          'button',
          {
            key: mode,
            onClick: () => form.setFieldValue('quantization', mode),
          },
          mode,
        ),
      ),
      createElement(SamplingModeSelect, {
        value: settings.samplingMode,
        onChange: (mode) => form.setFieldValue('samplingMode', mode),
      }),
  ...
  const click = (label: string) =>
    [...fixture.querySelectorAll('button')].find((button) => button.textContent === label)!.click();
  const payload = () =>
    [...fixture.querySelectorAll('dt')].find((term) => term.textContent === 'Encoded payload')?.nextElementSibling
      ?.textContent;
  try {
    root.render(createElement(Harness));
    await expect.poll(payload).toContain('2,550 bytes');
    const work = () =>
      [...fixture.querySelectorAll('dt')].find((term) => term.textContent === 'MLP work per sample')?.nextElementSibling
        ?.textContent;
    await expect.poll(work).toBe('1.2K FLOP · 1 evaluation');
    const select = fixture.querySelector('select')!;
    for (const [mode, expected] of [
      ['trilinear', '9.4K FLOP · 8 evaluations'],
      ['stochastic', '1.2K FLOP · 1 evaluation'],
      ['nearest', '1.2K FLOP · 1 evaluation'],
    ]) {
      select.value = mode;
