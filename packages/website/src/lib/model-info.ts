...
export interface ModelInfoData {
  name: string;
  size: Pick<
    ReturnType<typeof estimateModelSize>,
    'memoryBytes' | 'storageBytes' | 'mlpParams' | 'inputSize' | 'flopsPerDecode'
  >;
...
export function configuredModelInfo(
  settings: ModelSizeSettings,
  outputChannels: number,
  name: string,
  classification?: NTCChannelClassification | null,
): ModelInfoData {
  ...
  const grids: ModelInfoData['grids'] = resolutions.map((r) => ({
    group: 'G0',
    width: r,
    height: r,
    channels: 4,
    bits: size.gridStorageBits,
  }));
  if (settings.dualGrid)
    grids.push(
      ...resolutions.map((r) => ({
        group: 'G1' as const,
        width: Math.max(1, Math.floor(r / 2)),
        height: Math.max(1, Math.floor(r / 2)),
        channels: 4,
        bits: size.gridStorageBits,
      })),
    );
  ...
  return {
    ...
    size,
    grids,
    layers: widths.slice(1).map((outputSize, i) => ({ inputSize: widths[i], outputSize })),
    ...channelLists(classification),
    trainingQuantizationOff: settings.quantization === 'none',
  };
}
