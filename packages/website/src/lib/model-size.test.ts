  ...
  for (const dualGrid of [false, true]) {
    it.each(['none', 'uint8', 'uint4', 'uint2'] as const)(
      ...
      (quantization) => {
        ...
        const model = createNTCGridPyramidModel(
          {
            ...
            channels: 4,
            outputChannels: 3,
            hiddenSizes: [settings.hiddenSize],
          },
          () => 0.5,
        );
        ...
        const parameters = model.decoder.layers.reduce(
...
it('quantization changes packed storage, not the decoded runtime representation', () => {
  const sizes = ['uint8', 'uint4', 'uint2'].map((quantization) =>
    estimateModelSize({ ...defaults, quantization } as ModelSizeSettings, 3),
  );
...
it.each(['nearest', 'stochastic', 'trilinear'] as const)(
  'counts all MLP evaluations for %s without changing model size',
  (samplingMode) => {
