...
it('reports matching model details before and after export, including both grids and active-only channel count', () => {
  const settings: ModelSizeSettings = {
    baseResolution: 16,
    levels: 2,
    hiddenSize: 16,
    hiddenLayers: 1,
  ...
  const classification = {
    ...
    constantValues: {
      ...Object.fromEntries(CHANNELS.map((channel) => [channel.key, channel.defaultValue])),
      metalness: 0.7,
    },
  };
  const model = createNTCGridPyramidModel(
    { ...settings, channels: 4, outputChannels: 3, hiddenSizes: [16] },
    () => 0.5,
  );
