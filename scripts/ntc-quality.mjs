...
if (!cwd) {
  ...
  const files = execFileSync('git', ['ls-files', '-c', '-o', '--exclude-standard'], { cwd: root, encoding: 'utf8' })
    .trim()
    .split('\n');
...
const child = spawn(
  ...
  [
    ...
    'gpu',
    '--no-file-parallelism',
    'packages/three-ntc-trainer/src/NTCQuality.gpu.test.ts',
  ],
  { cwd, env: process.env },
);
let log = '';
...
if (code || records.length !== 6)
  throw new Error(`Benchmark failed (${code}, ${records.length} records); see docs/metrics/${label}.log`);
