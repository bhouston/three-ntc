...
const source = await readFile(new URL('../packages/three-ntc-trainer/src/NTCMipFilter.ts', import.meta.url), 'utf8');
const js = ts.transpileModule(source, {
  compilerOptions: { module: ts.ModuleKind.ESNext },
}).outputText;
const { createLanczosMipChain } = await import('data:text/javascript;base64,' + Buffer.from(js).toString('base64'));
const records = [];
